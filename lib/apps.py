...
nonorg_probability_gen = WeightedRandomGenerator(
    [0.52, 0.4, 0.2, 0.17, 0.16, 0.15], [7, 3, 20, 23, 5, 25])

# Time between opening application:
session_delay_gen = WeightedRandomGenerator([h * 3600 for h in range(1, 11)],
                                            sorted(
                                                numpy.random.beta(8, 8, 10)))

# Different inapp events number per app:
app_events_num_gen = RandomGenerator(
    [int(r) + 1 for r in numpy.random.beta(3, 10, 100) * 10])
...
events_per_session_gen = RandomGenerator(
    [int(r) for r in numpy.random.beta(1, 4, 100) * 10])
